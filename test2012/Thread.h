...
public:
   ...
   ~CThreadT()
   {
      if( t_bManaged ) ReleaseThread();
   }

   // Operations

   BOOL CreateThread(LPTHREAD_START_ROUTINE pThreadProc, LPVOID pParam = NULL, int iPriority = THREAD_PRIORITY_NORMAL)
   {
   ...
   void AttachThread(HANDLE hThread)
   {
      _ASSERTE(m_hThread==NULL);
      m_hThread = hThread;
   }
...
public:
   ...
   BOOL StartThread()
   {
      if( !_ClearAbort() ) 
	  {
		  ATLASSERT(NULL); return FALSE;
	  }
      if( !CreateThread(ThreadProc, (LPVOID) static_cast<T*>(this)) )
	  {
		  ATLASSERT(NULL); return FALSE;
	  }
      return TRUE;
   ...
   BOOL AbortThread()
   {
      ...
      if( m_bSuspended ) ResumeThread();
      return TRUE;
   }

   BOOL IsThreadAborted() const
   {
      _ASSERTE(m_hThread);
      return _IsAborted();
   }

   void SetThreadAutoClean(bool bAutoClean = true)
   {
      m_bAutoCleanup = bAutoClean;
   }

   void SetThreadDeleteOnExit(bool bAutoDelete = true)
   {
...
public:
   ...
   BOOL StartThread()
   {
      ...
      BOOL bRes = CThreadImpl<T, CThreadStopAtEvent>::StartThread();
